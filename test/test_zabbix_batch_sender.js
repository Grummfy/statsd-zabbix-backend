...
const { ZabbixBatchSender, Utils } = require('../lib/zabbix-batch-sender');
...
describe('zabbix sender utils - zabbix status manuipulation', () => {
  it('parses zabbix status', () => {
    const statusString = { info: 'processed: 56; failed: 35; total: 91; seconds spent: 0.005022' };
  ...
  it('merges zabbix statuses', () => {
    const statusString1 = { info: 'processed: 56; failed: 35; total: 91; seconds spent: 0.005022' };
    const statusString2 = { info: 'processed: 22; failed: 12; total: 7; seconds spent: 0.003425' };
...
describe('zabbix batch sender', () => {
  const senderFactory = (opts) => {
    const zabbixSender = Object.assign({
      failedCount: 0,
      itemsCount: 0,
      sendCount: 0,
      timeSpent: 0.0,
    ...
    zabbixSender.addItem = () => {
      zabbixSender.itemsCount += 1;
    };
    zabbixSender.clearItems = () => {
      zabbixSender.itemsCount = 0;
    };
    zabbixSender.send = (callback) => {
      zabbixSender.sendCount += 1;
      const statusString = `processed: ${zabbixSender.itemsCount - zabbixSender.failedCount}; failed: ${zabbixSender.failedCount}; total: ${zabbixSender.itemsCount}; seconds spent: ${zabbixSender.timeSpent.toFixed(6)}`;
      callback(null, { info: statusString });
    };
    return zabbixSender;
  };
