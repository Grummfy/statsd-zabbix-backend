 ...
 *                         [default: undefined]
 *   zabbixFilters:        List of pluggable metrics filtering scripts. If not configured,
 ...
 * @returns {array} List of {function(itemType, filteredItems)}.
 */
function allFilters() {
 ...
 * @param {string} itemType Specifies type of metrics in items array.
 ...
 */
function filterItems(itemType, items) {
  if (!filters || filters.length === 0) {
    return items;
  }
  let filteredItems = items;
  filters.forEach((filter) => {
    filteredItems = filter(itemType, filteredItems);
  });
...
function flush(targetBuilder, sender, flushInterval, timestamp, metrics) {
  ...
  const handle = (processor, filter, stat, value) => {
    try {
      const { host, key } = targetBuilder(stat);
      filter(processor(host, key, value)).forEach((item) => {
  ...
  const counterFilter = filterItems.bind(undefined, 'counter');
  Object.keys(metrics.counters).forEach((stat) => {
    handle(counterProcessor, counterFilter, stat, metrics.counters[stat]);
  });

  const timerProcessor = itemsForTimer.bind(undefined, metrics.pctThreshold);
  const timerFilter = filterItems.bind(undefined, 'timer');
  Object.keys(metrics.timers).forEach((stat) => {
    handle(timerProcessor, timerFilter, stat, metrics.timers[stat]);
  });

  const guageFilter = filterItems.bind(undefined, 'guage');
  Object.keys(metrics.gauges).forEach((stat) => {
    handle(itemsForGauge, guageFilter, stat, metrics.gauges[stat]);
  });
...
function init(startupTime, config, events, l) {
  ...
  const loadedFilters = [];
  if (config.zabbixFilters && config.zabbixFilters.length) {
    config.zabbixFilters.forEach((filter) => {
      const filterFactory = require(filter); // eslint-disable-line global-require, import/no-dynamic-require, max-len
      const dynFilter = filterFactory(config, l);
      loadedFilters.push(dynFilter);
    });
  }
