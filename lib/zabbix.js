 ...
 * @param {ZabbixSender} constructSender Function that constructs an instance if Zabbix Batch Sender for sending stats to Zabbix.
 * @param {number} flushInterval How long stats were collected, for calculating average.
 * @param {number} timestamp Time of flush as unix timestamp.
 * @param {Object} metrics Metrics provided by StatsD.
 * @returns {undefined}
 */
function flush(targetBuilder, constructSender, publisher, flushInterval, timestamp, metrics) {
  ...
  const sender = constructSender();
  publisher(allItems, sender, function(response){
...
function init(startupTime, config, events, l) {
  ...
  const constructSender = function(){
    return new BatchSender.ZabbixBatchSender({
  ...
  events.on('flush', flush.bind(undefined, targetBuilder, constructSender, publisher, config.flushInterval));
