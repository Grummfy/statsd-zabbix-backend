...
function prependPublishStats(items, state) {
  if (state.publishStats) {
    ...
    if (host) {
      const publishItems = itemsForStats(localState.publishStats.metrics, host, localState.prefixStats, 'metric');
      publishItems.forEach((it) => {
        items.unshift(it);
      });
      const discoveryItems = itemsForStats(localState.publishStats.discovery, host, localState.prefixStats, 'descovery');
      discoveryItems.forEach((it) => {
        items.unshift(it);
      });
    }
  }
...
function updatePublishStats(stats, zabbixState) {
  stats.processed = (stats.processed || 0) + (zabbixState.processed || 0);
  stats.failed = (stats.failed || 0) + (zabbixState.failed || 0);
  stats.total = (stats.total || 0) + (zabbixState.total || 0);
  stats.secondsSpent = (stats.secondsSpent || 0.0) + (zabbixState.secondsSpent || 0.0);
}
...
function publishItems(state, items, batchSender) {
  ...
  items.forEach((item) => {
    if (shouldSendMetric(item)) {
      ...
      if (!localState.discovered[item.key]) {
        ...
        if (discoveryArray.length >= state.maxDiscoveryBatchSize){
          ...
          batchSender.publishBatch([discoveryItem], (response) => {
    ...
    const wrappedItem = wrapMetric(item);
