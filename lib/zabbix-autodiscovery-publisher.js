...
function itemsForStats(stats, host, prefix, name) {
  ...
  if (stats.processed) {
    items.push({
      host,
      key: `${prefix}.zabbix_publisher.${name}_processed`,
  ...
  if (stats.failed) {
    items.push({
      host,
      key: `${prefix}.zabbix_publisher.${name}_failed`,
  ...
  if (stats.total) {
    items.push({
      host,
      key: `${prefix}.zabbix_publisher.${name}_total`,
      value: stats.total || 0,
    });
  }
  if (stats.secondsSpent) {
    items.push({
      host,
      key: `${prefix}.zabbix_publisher.${name}_timespent`,
  ...
  if (state.reportPublishStats && state.publishStats.length > 0) {
    ...
    if (host) {
      while (state.publishStats.length > 0) {
        ...
        const publishStatsItems = itemsForStats(publishStats.metrics, host, localState.prefixStats, 'metric');
        publishStatsItems.forEach((it) => {
          items.unshift(it);
        });
        const discoveryStatsItems = itemsForStats(publishStats.discovery, host, localState.prefixStats, 'discovery');
